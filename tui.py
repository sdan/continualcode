...
Minimal CLI for SDPO online learning.
"""
...
async def _shimmer(label: str, stop: asyncio.Event) -> None:
    ...
    while not stop.is_set():
        ...
        # Trail length based on distance from nearest edge — grows in the middle, shrinks at edges
...
def _print_user(msg: str) -> None:
    # Keep the prompt line as the only user echo for simplicity.
    return
...
async def main() -> None:
    print(f"{BOLD}sdpo_simple_tui{RESET} {DIM}| {MODEL} | {os.getcwd()}{RESET}")
    ...
    _print_system(f"Initializing SDPO session ({mode})...")

    sdpo_config = SDPOConfig(kl_coef=SDPO_KL_COEF, is_clip=SDPO_IS_CLIP)
    session = SDPOSession(
        model=MODEL,
    ...
    if ENABLE_TRAINING and not ENABLE_RL_TRAINING:
        ...
        try:
            ...
            while True:
                ...
                if name == "edit_lines":
                    ...
                    old_content = model_args.get("old_content")  # May not exist

                    # Try to read current file content for old lines
                    if not old_content and path and os.path.isfile(path):
                        try:
                            with open(path, "r", encoding="utf-8", errors="replace") as f:
                                file_lines = f.readlines()
                            # Extract the lines being replaced (0-indexed)
